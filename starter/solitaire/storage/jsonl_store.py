...
Append-only JSONL files as canonical persistence. SQLite is session-local cache only.

Design principles:
  1. JSONL on mount = single source of truth
  2. Append-only: no overwrites, no deletes on disk
  3. Every append flushes the index (no batching, no stale windows)
  4. Session-local SQLite is disposable — rebuilt from JSONL on boot
