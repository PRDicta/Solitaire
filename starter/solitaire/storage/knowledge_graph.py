...
CREATE INDEX IF NOT EXISTS idx_entity_name_lower ON entity_nodes(name_lower);
...
class KnowledgeGraph:
    ...
    def add_edge(
        ...
        # Check if edge already exists (same source, target, relationship)
        ...
        if existing:
            ...
            edge_id = existing["id"] if isinstance(existing, sqlite3.Row) else existing[0]
            weight = existing["weight"] if isinstance(existing, sqlite3.Row) else existing[1]
            self.conn.execute(
                "UPDATE entity_edges SET weight = ?, ingestion_time = ? WHERE id = ?",
                (weight + 1.0, now, edge_id)
            )
            return edge_id
