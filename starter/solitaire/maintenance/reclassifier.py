...
Tier 3 (medium confidence): Assistant-authored entries attributed to the user.
...
_DURABLE_INTENT_PATTERNS = [
    # Owner/User + strong directive verbs (decisions, preferences, instructions)
    re.compile(r"\b{owner}\b.*\bprefers?\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
    re.compile(r"\b{owner}\b.*\bdecided\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
    re.compile(r"\b{owner}\b.*\bconfirmed\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
    re.compile(r"\b{owner}\b.*\bapproved\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
    re.compile(r"\b{owner}\b.*\bcorrected\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
    re.compile(r"\b{owner}\b.*\binstructed\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
    re.compile(r"\b{owner}\b.*\brejected\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
...
_WANTS_PATTERNS = [
    re.compile(r"\b{owner}\b.*\bwants?\s+to\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
    re.compile(r"\bUser\b.*\bwants?\s+to\b", re.IGNORECASE),
    re.compile(r"\b{owner}\b.*\bwants?\b", re.IGNORECASE),  # Note: {owner} is replaced at runtime with owner_name variable
    ...
    for row in rows:
        ...
        if durable_matches and not _is_ephemeral(content):
            ...
            results.append(Reclassification(
                ...
                tier=3,
                confidence=0.75,
                reason="Attributed entry with durable directive language",
