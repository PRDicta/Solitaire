...
class TopicRouter:
    ...
    def __init__(
        ...
        self._jsonl_metadata_store = None
        self._jsonl_session_id = ""

    def attach_jsonl_store(self, metadata_store, session_id: str = ""):
        """Attach a JsonlStore for metadata.jsonl canonical persistence."""
        self._jsonl_metadata_store = metadata_store
    ...
    async def create_topic(
        ...
        # JSONL: append topic to metadata
        if self._jsonl_metadata_store:
            try:
                self._jsonl_metadata_store.append(
                    record_type="topic",
                    op="create",
                    data={
                        "id": topic_id, "label": label, "description": description,
                        "created_at": now, "entry_count": len(seed_entries) if seed_entries else 0,
                    },
    ...
    def _record_assignment(
        ...
        # JSONL: record assignment
        if self._jsonl_metadata_store:
            try:
                assign_id = f"ta:{entry_id}:{topic_id}"
                self._jsonl_metadata_store.append(
                    record_type="topic_assignment",
                    op="create",
                    data={
                        "entry_id": entry_id, "topic_id": topic_id,
