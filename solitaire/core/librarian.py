...
With an LLM adapter (optional enhancement):
    ...
    from solitaire.indexing.anthropic_adapter import AnthropicAdapter
...
class TheLibrarian:
    ...
    def __init__(
        ...
        self.disposition_filter: Optional[DispositionFilter] = None
        if persona_path:
            try:
                self.persona = PersonaProfile.from_yaml(persona_path)
            except Exception:
                self.persona = None
        ...
        # ─── Initialize Disposition Filter (after session is started) ─
        if self.persona:
            self.disposition_filter = DispositionFilter(
                persona=self.persona,
                session_id=self.state.conversation_id,
            )
            # Load custom signals from persona.yaml
            if self.persona.custom_signals:
                try:
                    loaded = self.disposition_filter.load_custom_signals(
                        self.persona.custom_signals
                    )
                except Exception:
                    pass  # Custom signals are a bonus, never a blocker
            # Apply drift overlay from rolodex on boot
            self._apply_persona_drift_overlay()
    ...
    async def ingest(
        ...
        # ─── Disposition Filter: live signal detection ────────────
        if self.disposition_filter:
            try:
                self._run_disposition_filter(content, role)
            except Exception:
                pass  # Persona is a bonus, never a blocker
    ...
    async def retrieve_with_scores(
        ...
        scores = {}
        if response.entries:
            # Score based on position — top result = 1.0, decays by 0.1 per rank.
            # HybridSearcher already ranks by relevance, so position is the
            # best proxy available. Decay of 0.1 preserves meaningful separation.
            for i, entry in enumerate(response.entries):
                scores[entry.id] = max(0.1, 1.0 - (i * 0.1))
    ...
    def _apply_persona_drift_overlay(self) -> None:
        """Load disposition_drift entries from rolodex and overlay on persona baseline."""
        if not self.persona:
            return
        try:
            import json
            from .persona import DriftEntry
            rows = self.rolodex.conn.execute(
                "SELECT content, metadata, conversation_id, created_at "
            ...
            # Build session index for decay calculation
            ...
            for row in rows:
                try:
                    ...
                    entry = DriftEntry.from_content_dict(
                        content_data,
            ...
            self.persona.apply_drift_overlay(
                drift_entries=drift_entries,
                current_session_count=current_session_count,
                session_counts=session_index,
            )
        except Exception:
            pass  # Graceful degradation — persona works without drift

    def _run_disposition_filter(self, content: str, role: str) -> None:
        """Run the disposition filter on an ingested message and store drift entries."""
        if not self.disposition_filter:
            return

        import json
        from .types import EntryCategory, ContentModality, Tier

        # Fetch reinforcement counts from DB
        reinforcement_counts = {}
        try:
            query = self.disposition_filter.get_reinforcement_counts_query()
            rows = self.rolodex.conn.execute(query).fetchall()
            for row in rows:
                signal = row["signal"]
                trait = row["trait"]
                count = row["count"]
                if signal not in reinforcement_counts:
                    reinforcement_counts[signal] = {}
                reinforcement_counts[signal][trait] = count
        except Exception:
            pass  # Proceed without reinforcement data

        # Evaluate
        drift_entries = self.disposition_filter.evaluate(
            content=content,
            role=role,
            reinforcement_counts=reinforcement_counts,
        )

        # Store drift entries in rolodex
        for drift in drift_entries:
            entry = RolodexEntry(
                conversation_id=self.state.conversation_id,
                content=json.dumps(drift.to_content_dict()),
                content_type=ContentModality.STRUCTURED,
                category=EntryCategory.DISPOSITION_DRIFT,
                tags=["persona", "drift", drift.signal],
                tier=Tier.COLD,
                metadata={
                    "signal_type": drift.signal,
                    "confidence": drift.confidence,
                    "reinforcement_count": drift.reinforcement_count,
                },
            )
            self.rolodex.create_entry(entry)

    def get_persona_profile(self) -> Optional[Dict[str, Any]]:
        """Return the current persona state for CLI display."""
        if not self.persona:
            return None
        return {
            "identity": self.persona.identity.to_dict(),
            "effective_traits": self.persona.traits.to_dict(),
            "baseline_traits": self.persona.baseline.to_dict(),
            "drift_delta": self.persona.get_drift_delta(),
            "domain": self.persona.domain.to_dict(),
            "sharing": self.persona.sharing.to_dict(),
            "ascii_profile": self.persona.format_ascii_profile(),
        }
