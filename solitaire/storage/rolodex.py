...
from ..core.reinforcement import on_entry_created as _reinforcement_hook
from ..core.temporal_reasoning import TemporalReasoner as _TemporalReasoner
from .schema import (
...
class SharedStore:
    """Shared profile store that lives in a separate DB from persona rolodexes.

    Holds user_profile data that is persona-independent (name, timezone,
    preferences, etc.). Each persona's Rolodex has its own DB; this store
    is the cross-persona shared layer.
    """

    def __init__(self, db_path: str = "shared.db"):
        self.db_path = db_path
        self.conn = init_shared_database(db_path)
...
class Rolodex:
    ...
    def __init__(self, db_path: str = "rolodex.db"):
        ...
        # JSONL canonical store (attached externally after construction)
        self._jsonl_store = None  # Optional[PersonaJsonlStore]
    ...
    def _jsonl_append(self, record_type: str, op: str, record_id: str, data: Dict[str, Any]):
        """Append a record to the attached JSONL store (entries file). No-op if not attached."""
        if not self._jsonl_store:
            return
        try:
            self._jsonl_store.entries.append(
            ...
            pass  # JSONL append failure is non-fatal; SQLite is the live query path
    # ─── Write Operations ────────────────────────────────────────────────
    def create_entry(self, entry: RolodexEntry) -> str:
        ...
        self.conn.commit()
        # JSONL: append create record
        entry_data = {k: v for k, v in zip(
            ["id", "conversation_id", "content", "content_type", "category",
             "tags", "source_range", "access_count", "last_accessed",
             "created_at", "tier", "embedding", "linked_ids", "metadata"],
            values
        )}
        entry_data["verbatim_source"] = 1 if entry.verbatim_source else 0
        entry_data["provenance"] = getattr(entry, 'provenance', None) or 'unknown'
        self._jsonl_append("rolodex_entry", "create", entry.id, entry_data)
        # Add to hot cache (most recent entries are likely to be needed)
        self._cache_put(entry)
        # Hindsight: stamp initial confidence and reinforce overlapping entries
        try:
            _reinforcement_hook(
                conn=self.conn,
                entry_id=entry.id,
                ...
                provenance=getattr(entry, 'provenance', 'unknown') or 'unknown',
            )
        except Exception:
            pass  # Reinforcement failure is non-fatal
        # Hindsight: extract temporal signals (status changes, decisions, completions)
        try:
            tr = _TemporalReasoner(self.conn)
            tr.process_entry_temporal(
                content=entry.content,
                entry_id=entry.id,
                created_at=entry.created_at.isoformat() if entry.created_at else None,
            )
        except Exception:
            pass  # Temporal extraction failure is non-fatal
        return entry.id
    def batch_create_entries(self, entries: List[RolodexEntry]) -> List[str]:
        """Bulk insert entries. Returns list of IDs."""
        # Enable batch mode on JSONL to defer index flush until commit
        if self._jsonl_store:
            self._jsonl_store.entries.set_batch_mode(True)
        ids = []
        ...
        # Flush JSONL batch
        if self._jsonl_store:
            self._jsonl_store.entries.set_batch_mode(False)
        return ids
    ...
    def update_entry_enrichment(
        ...
        verbatim_source: Optional[bool] = None,
        ...
        if verbatim_source is not None:
            updates.append("verbatim_source = ?")
            params.append(1 if verbatim_source else 0)
        ...
        if embedding is not None:
            from .jsonl_store import encode_embedding
            update_data["embedding"] = encode_embedding(embedding)
        if metadata is not None:
            update_data["metadata"] = metadata
        if verbatim_source is not None:
            update_data["verbatim_source"] = 1 if verbatim_source else 0
        if provenance is not None:
            update_data["provenance"] = provenance
        self._jsonl_append("rolodex_entry", "update", entry_id, update_data)
    ...
    def archive_entry(self, entry_id: str, reason: str = "") -> bool:
        """
        Archive an entry: sets archived_at timestamp, excludes from search.
        The entry is preserved for timeline reconstruction but invisible to
        FTS and semantic search. Use for superseded decisions, outdated info,
        or experimental entries that shouldn't pollute retrieval.
        ...
        if result.rowcount > 0:
            self.conn.commit()
            # JSONL: append archive record
            self._jsonl_append("rolodex_entry", "archive", entry_id,
                               {"id": entry_id, "archived_at": now, "reason": reason})
    ...
    def unarchive_entry(self, entry_id: str) -> bool:
        ...
        result = self.conn.execute(
            "UPDATE rolodex_entries SET archived_at = NULL WHERE id = ? AND archived_at IS NOT NULL",
            (entry_id,)
        )
        if result.rowcount > 0:
            self.conn.commit()
            # JSONL: append unarchive record
            self._jsonl_append("rolodex_entry", "update", entry_id,
                               {"id": entry_id, "archived_at": None})
            return True
        return False

    def delete_entry(self, entry_id: str) -> bool:
        """
        Hard-delete an entry. Use for flatly incorrect information
        (wrong name, wrong location, fabricated details). No recovery.
        Also cleans up FTS index and topic assignments.
        """
        self.conn.execute("DELETE FROM rolodex_fts WHERE entry_id = ?", (entry_id,))
        self.conn.execute("DELETE FROM topic_assignments WHERE entry_id = ?", (entry_id,))
        result = self.conn.execute("DELETE FROM rolodex_entries WHERE id = ?", (entry_id,))
        if result.rowcount > 0:
            self.conn.commit()
            # JSONL: append delete record
            self._jsonl_append("rolodex_entry", "delete", entry_id, {"id": entry_id})
            self._hot_cache.pop(entry_id, None)
            return True
        return False
    ...
    def _sanitize_fts_query(query: str) -> str:
        ...
        FTS5 treats hyphens as the NOT operator, so 'self-observation'
        becomes 'self NOT observation' which errors or returns wrong results.
        Replace intra-word hyphens with spaces so hyphenated terms become
        AND queries instead. Also strip other FTS5 special characters that
        could cause parse errors.

        Characters that must be stripped:
        - Hyphens (intra-word): FTS5 NOT operator
        - Double quotes: FTS5 phrase delimiter
        - Asterisks: FTS5 prefix wildcard
        - Carets: FTS5 initial token operator
        - Question marks: FTS5 syntax error
        - Apostrophes/single quotes: FTS5 unterminated string literal
        - Parentheses: FTS5 grouping operators
        - Colons: FTS5 column filter operator
        - Semicolons, exclamation marks: potential parse errors
        """
        import re as _re
        # Replace hyphens between word characters with spaces (keeps "NOT" semantic for standalone -)
        sanitized = _re.sub(r'(?<=\w)-(?=\w)', ' ', query)
        # Strip all FTS5 special chars and punctuation that causes parse errors
    ...
    def _fts_match(
        ...
        Args:
            category_filter: Optional list of category strings to restrict results.
                When provided, only entries whose category is in this list are returned.
                Used by Tier 2 recall to target specific knowledge categories.
        """
        ...
        if category_filter:
            ...
            sql += f" AND re.category IN ({placeholders})"
    ...
    def create_chain(self, chain: ReasoningChain) -> str:
        ...
        self.conn.commit()
        # JSONL: append chain to chains file
        if self._jsonl_store:
            try:
                from .jsonl_store import encode_embedding
                chain_data = {
                    ...
                    "turn_range_start": chain.turn_range_start,
                    "turn_range_end": chain.turn_range_end,
                    "summary": chain.summary,
                    "topics": chain.topics,
                    "related_entries": chain.related_entries,
                    "embedding": encode_embedding(chain.embedding),
                    "created_at": chain.created_at.isoformat() if hasattr(chain.created_at, 'isoformat') else str(chain.created_at),
                }
                self._jsonl_store.chains.append(
                    record_type="chain",
                    op="create",
                    data=chain_data,
                    session_id=self._jsonl_session_id,
    ...
    def get_user_knowledge_entries(self, priority: str = None) -> List[RolodexEntry]:
        """Fetch user_knowledge entries, optionally filtered by priority.

        Priority levels (stored in metadata JSON field "priority"):
          - "core": Boot-loaded. Identity, behavioral rules, response prefs.
          - "reference": Recall-on-demand. Project facts, historical context.
          - None/unset: Treated as "reference" for boot filtering.

        Args:
            priority: If "core", return only core entries for boot.
                      If "reference", return only reference entries.
                      If None, return all (for recall/search).
        """
        if priority == "core":
            # Optimized query: filter in SQL via JSON metadata
            rows = self.conn.execute(
                """SELECT * FROM rolodex_entries
    ...
    def supersede_entry(self, old_entry_id: str, new_entry_id: str) -> bool:
        ...
        # JSONL: append supersede record
        self._jsonl_append("rolodex_entry", "supersede", old_entry_id,
                           {"id": old_entry_id, "superseded_by": new_entry_id})
    ...
    def profile_set(self, key: str, value: str, session_id: Optional[str] = None) -> None:
        ...
        self.conn.commit()
        # JSONL: append to metadata file
        if self._jsonl_store:
            try:
                self._jsonl_store.metadata.append(
                    record_type="user_profile",
                    op="upsert",
                    data={"id": f"profile:{key}", "key": key, "value": value,
                          "source_session": session_id, "updated_at": now},
                    session_id=self._jsonl_session_id,
                    record_id=f"profile:{key}",
                )
            except Exception:
                pass
    ...
    def profile_delete(self, key: str) -> bool:
        ...
        if cursor.rowcount > 0:
            # JSONL: append delete to metadata
            if self._jsonl_store:
                try:
                    self._jsonl_store.metadata.append(
                        record_type="user_profile",
                        op="delete",
                        data={"id": f"profile:{key}", "key": key},
                        session_id=self._jsonl_session_id,
                        record_id=f"profile:{key}",
                    )
                except Exception:
        ...
        if self._jsonl_store:
            try:
                self._jsonl_store.metadata.append(
                    ...
                    op="create",
                    data={
                        "id": doc_id, "file_name": file_name, "file_path": file_path,
                        "file_type": file_type, "file_hash": file_hash, "title": title,
                        "page_count": page_count, "summary": summary,
                        "registered_at": now, "metadata": metadata or {},
                    },
                    session_id=self._jsonl_session_id,
                    record_id=doc_id,
                )
    ...
    def update_entry_document_source(
        ...
        # JSONL: append update record
        self._jsonl_append("rolodex_entry", "update", entry_id,
                           {"id": entry_id, "source_type": "document",
                            "document_id": document_id, "source_location": source_location})
