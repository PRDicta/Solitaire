#!/usr/bin/env python3
"""Claude Code Stop hook: scan assistant responses for unverified state claims.

Drop-in hook for Claude Code's Stop event. Reads the session transcript,
extracts the most recent assistant response, and scans for unverified
assertions about remote/unobserved system state.

If claims are detected, writes a JSON marker file that the preflight
evaluation gate picks up on the next turn to inject a "Stop. Think.
Check. Be Sure." block.

Setup: Add to .claude/settings.json alongside auto-ingest:
{
  "hooks": {
    "Stop": [{
      "matcher": "",
      "hooks": [
        {
          "type": "command",
          "command": "python <path-to>/claude-code-claim-scanner.py",
          "timeout": 30
        }
      ]
    }]
...
_STATE_CLAIMS = [
    re.compile(
        r"\b(?:the|her|his|their|Bernie'?s?|Brenna'?s?)\s+\w+\s+"
    ...
    re.compile(
        ...
        r"(?:is|are)\s+"
        r"(?:broken|corrupt|missing|outdated|misconfigured|incompatible|borked|messed\s+up)",
        re.IGNORECASE,
    ),
    re.compile(
...
_FILE_OP_RECS = [
    re.compile(
        ...
        r"\s*(?:delete|remove|move|rename|overwrite|replace|copy|nuke|wipe)\b"
        r".{0,60}"
...
def has_remote_context(text):
    """Check if the text involves remote/unobserved systems."""
    return any(p.search(text) for p in _REMOTE_CONTEXT)


def has_verification(text):
    """Check if the text contains verification signals."""
    return any(p.search(text) for p in _VERIFICATION)


def scan_for_claims(text):
    """Scan assistant text for unverified state claims.

    Returns list of dicts with category and matched text, or empty list.
    """
    if not text or len(text) < 20:
        return []

    # Only scan if the response involves remote/unobserved systems
    if not has_remote_context(text):
        return []

    # If the response contains verification language, suppress
    ...
    for pattern in _FILE_OP_RECS:
        ...
        if m:
            claims.append({
                "category": "file_op_recommendation",
                "text": m.group(0)[:100],
            })
    ...
    for pattern in _NARRATIVE:
        ...
        if m:
            claims.append({
                "category": "narrative_construction",
                "text": m.group(0)[:100],
            })
...
def write_marker(claims, workspace=None):
    """Write a marker file for the preflight to pick up."""
    ...
    try:
        ...
        texts = [c["text"] for c in claims[:3]]  # Cap summary at 3
