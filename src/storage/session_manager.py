    ...
    def list_sessions(self, limit: int = 20) -> List[SessionInfo]:
        ...
        for row in rows:
            sessions.append(SessionInfo(
                session_id=row["id"],
                started_at=_safe_fromisoformat(row["created_at"]),
                last_active=(
                    _safe_fromisoformat(row["last_active"])
                    if row["last_active"] else None
                ),
                ended_at=(
                    _safe_fromisoformat(row["ended_at"])
    ...
    def load_messages(self, conversation_id: str) -> List[Message]:
        ...
        for row in rows:
            msg = Message(
                ...
                timestamp=_safe_fromisoformat(row["timestamp"]),
            )
            messages.append(msg)
        return messages
