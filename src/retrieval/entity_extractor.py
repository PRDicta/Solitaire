...
# Named person attribution (e.g., "the user's analogy", "the user said")
...
# Module-level cache for DB-loaded proper nouns (populated once per session)
_DB_PROPER_NOUNS: Set[str] = set()
_DB_NOUNS_LOADED = False


def load_proper_nouns_from_db(conn) -> None:
    """Load entity names from the knowledge graph into the proper noun set.

    Called once per session (during auto-recall setup). Entries from
    entity_nodes with mention_count >= 2 are added to the runtime set.
    This bridges the gap between the static bootstrap list and entities
    discovered through ingestion.
    """
    global _DB_PROPER_NOUNS, _DB_NOUNS_LOADED
    if _DB_NOUNS_LOADED:
        return
    try:
        rows = conn.execute(
            "SELECT name FROM entity_nodes WHERE mention_count >= 2"
        ).fetchall()
        for row in rows:
            name = row[0] if not isinstance(row, dict) else row["name"]
            _DB_PROPER_NOUNS.add(name.lower())
        _DB_NOUNS_LOADED = True
    except Exception:
        # Table may not exist yet — that's fine, use static set only
        _DB_NOUNS_LOADED = True
...
class EntityExtractor:
    ...
    def _detect_content_attribution(self, content: str) -> str:
        ...
        user_markers = [
            ...
            r"\b{owner}'?s?\b",  # Runtime substitution with owner_name variable
        ]
        ...
        for sentence in sentences:
            ...
            for i, word in enumerate(words):
                ...
                if clean[0].isupper():
                    ...
                    if i == 0 and lower not in _KNOWN_PROPER_NOUNS and lower not in _DB_PROPER_NOUNS:
                        continue
