...
def _extract_entity_name_from_context(content: str) -> Optional[str]:
    """
    Heuristic: extract a likely entity name from content.
    Looks for capitalized phrases and known patterns.
    """
    # Find the longest capitalized phrase at the beginning or after punctuation
    lines = content.split('\n')
...
def _detect_completions(content: str) -> List[Tuple[str, str, str]]:
    """
    ...
    for keyword in _COMPLETION_KEYWORDS:
        ...
        for match in pattern.finditer(content):
            ...
            completions.append(("Task", task, f"Completed: {task}"))
...
class TemporalReasoner:
    ...
    def process_entry_temporal(
        ...
        # Detect completions
        for entity_name, task_desc, description in _detect_completions(content):
            event = self.record_event(
                entity_name=entity_name,
                event_type="status_change",
                description=description,
