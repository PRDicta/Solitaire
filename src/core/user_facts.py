...
def build_facts_block(facts: List[UserFact]) -> str:
    ...
    Format is concise and scannable:
      ═══ KNOWN FACTS ═══
      {owner}: lives in California | works at Acme Corp | timezone is Pacific
      Acme Corp: is a content agency | founded in 2024
      Wife: name is Sarah
      ═══ END KNOWN FACTS ═══
    """
    if not facts:
