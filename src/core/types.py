...
class ContentModality(Enum):
    ...
    MIXED = "mixed"
    # Extended modalities (imported from external Librarian instances)
    INSIGHT = "insight"
    USER_KNOWLEDGE_MODALITY = "user_knowledge"
class EntryCategory(Enum):
    ...
    # Persona system — disposition drift observations
    DISPOSITION_DRIFT = "disposition_drift"
    # Extended categories (imported from external Librarian instances)
...
class RolodexEntry:
    ...
    # Bi-temporal: when the content describes (vs created_at = when ingested)
    event_time: Optional[datetime] = None     # NULL = same as created_at (contemporaneous)
    # Provenance: who asserted this content
    provenance: str = "unknown"               # user-stated | assistant-inferred | system | unknown
@dataclass
